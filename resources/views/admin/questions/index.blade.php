...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <h3 class="font-weight-bold mt-1 mb-0">Bank Soal Ujian</h3>
            <p class="text-secondary text-sm mb-0">Event: <strong>{{ $event->title }}</strong></p>
        </div>
        <a href="{{ route('admin.events.questions.create', $event->id) }}" class="btn btn-sm bg-gradient-success font-weight-bold">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card shadow-sm p-3 bg-white">
                <small class="text-muted d-block font-weight-bold">Total Soal Terdaftar</small>
                <h4 class="font-weight-bold mb-0 text-dark">{{ $questions->count() }} <small class="text-sm text-muted font-weight-normal">Soal</small></h4>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card shadow-sm p-3 bg-white">
                <small class="text-muted d-block font-weight-bold">Soal Pretest</small>
                <h4 class="font-weight-bold mb-0 text-warning">
                    {{ $questions->where('type', 'pretest')->count() }}
                    <small class="text-sm text-muted font-weight-normal">Soal</small>
                </h4>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card shadow-sm p-3 bg-white">
                <small class="text-muted d-block font-weight-bold">Soal Posttest</small>
    ...
    <div class="card shadow-sm overflow-hidden">
        <div class="card-header bg-white p-4 border-0">
            <h5 class="font-weight-bold mb-0">Daftar Pertanyaan Ujian</h5>
        </div>
        <div class="card-body p-4 pt-0">
            <div class="accordion accordion-flush" id="accordionQuestions">
                @forelse($questions as $index => $q)
                    <div class="accordion-item border mb-3 overflow-hidden">
                        <h2 class="accordion-header" id="heading{{ $q->id }}">
                            <button class="accordion-button collapsed px-3 py-3 bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $q->id }}">
                                <div class="d-flex align-items-center w-100 me-3">
                                    ...
                                    <span class="font-weight-bold text-dark me-2">#{{ $index + 1 }}</span>
                        ...
                        <div id="collapse{{ $q->id }}" class="accordion-collapse collapse" data-bs-parent="#accordionQuestions">
                            <div class="accordion-body p-4 bg-white">
                                <!-- Teks Pertanyaan -->
                                <p class="font-weight-bold text-dark text-sm mb-3">{{ $q->question_text }}</p>
                                ...
                                <div class="row g-2 mb-3">
                                    @if(is_array($q->options) || is_object($q->options))
                                        @foreach($q->options as $key => $opt)
                                            <div class="col-md-6">
                                                <div class="p-3 border d-flex justify-content-between align-items-center {{ strtolower($key) == strtolower($q->correct_answer) ? 'bg-success-subtle border-success font-weight-bold text-success' : 'bg-light text-dark' }}">
                                                    <div>
